            ...
            if ( @move_uploaded_file ( $arquivo_tmp, $destino ) ) {
                ...
                if(empty($requestData['TITULO'])){
                    ...
                    if($operacao == 'insert'){
                        ...
                        try{
                            $stmt = $pdo->prepare('INSERT INTO TRABALHO (TITULO, ANO, NUM_PAGINAS, RESUMO, ORIENTADOR, COORIENTADOR, ARQUIVO) VALUES (:a, :b, :c, :d, :e, :f, :g)');
                            $stmt->execute(array(
                                ':a' => utf8_decode($requestData['TITULO']),
                                ':b' => $requestData['ANO'],
                                ':c' => $requestData['NUM_PAGINAS'],
                        ...
                        try{
                            $stmt = $pdo->prepare('UPDATE TRABALHO SET TITULO = :a, ANO = :b, NUM_PAGINAS = :c, RESUMO = :d, ORIENTADOR = :e, COORIENTADOR = :f, ARQUIVO = :g WHERE IDTRABALHO = :id');
                            $stmt->execute(array(
                                ':id' => $ID,
                                ':a' => utf8_decode($requestData['TITULO']),
                                ':b' => $requestData['ANO'],
